...
class MusicRecommendationFungus:
    ...
    def start(self):
        ...
        i = 0
        while True:
            logging.info(f"[START] Starting epoch {i} (at {datetime.datetime.now()})")
            try:
    ...
    def answer_user_feedback(self):
        ...
        overall_favorites = self.mastodon_client.count_likes_of_all_statuses()
        if overall_favorites > 0:
            feedback = num_of_statuses_sent / overall_favorites
        else:
            feedback = 0
        return feedback
